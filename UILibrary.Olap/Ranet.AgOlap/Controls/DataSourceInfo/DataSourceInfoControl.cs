...
namespace Ranet.AgOlap.Controls.DataSourceInfo
{
    ...
    {
        RichTextBox connectionString;
        RichTextBox mdxQuery;
        RichTextBox movedAxes_MdxQuery;
        RichTextBox updateScript;
        CheckBox parseMdx;
        ...
        void CopyButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Clipboard.SetClipboardText(mdxQuery.Text);
                return;
            }
            if (queryInfoTabControl.TabCtrl.SelectedIndex == 1)
            {
                Clipboard.SetClipboardText(movedAxes_MdxQuery.Text);
                return;
            }
            if (queryInfoTabControl.TabCtrl.SelectedIndex == 2)
            {
                Clipboard.SetClipboardText(updateScript.Text);
                return;
            }
        }
