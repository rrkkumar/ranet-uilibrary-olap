...
namespace Ranet.Olap.Core.Data
{
    public class CellValueData : IPropertiesData
    {
        ...
        internal void Serialize(XmlWriter writer)
        {
            ...
            writer.WriteStartElement("CellValueData");
            // Свойства
            writer.WriteElementString("DisplayValue", this.DisplayValue.ToString(CultureInfo.InvariantCulture));
            if (this.Value != null)
            {
        ...
        internal static CellValueData Deserialize(XmlReader reader)
        {
            if (reader != null)
            {
                try
                {
                    CellValueData target = null;
                    if (reader.NodeType == XmlNodeType.Element &&
                        reader.Name == "CellValueData")
                    {
                        target = new CellValueData();

                        reader.ReadStartElement("CellValueData");

                        reader.ReadStartElement("DisplayValue");
                        if (reader.NodeType == XmlNodeType.Text)
                        {
                            target.DisplayValue = reader.ReadContentAsString();
